...
def missile_profile(name,x,z):
    # A real pointed projectile, retaining the reference's five distinct front cones.
    sections=[(-.74,.006),(-.66,.065),(-.53,.138),(-.40,.196),(-.22,.207),(.30,.207),(.48,.170)]
    n=32
    vertices=[(x+r*math.cos(i*math.tau/n),y,z+r*math.sin(i*math.tau/n)) for y,r in sections for i in range(n)]
    ...
    for row in range(len(sections)-1):
        for i in range(n):
            ...
            material_indices.append(1 if row==3 else 0)
    obj=v.mesh(name,vertices,faces,'titanium')
    obj.data.materials.append(v.M['red'])
    for face,index in zip(obj.data.polygons,material_indices):
        face.material_index=index
        face.use_smooth=len(face.vertices)==4
    for y in (-.15,.19,.39):
        cyl(name+'Collar',(x,y,z),.216,.035,'armor',(0,1,0))
    for sign in (-1,1):
        box(name+'Cradle',(x+sign*.185,-.04,z-.14),(.080,.52,.12),'edge',bevel=.023)
...
def build(item):
    ...
    scene['revision']='USER_APPROVED_SUPPLIES_V2'
    scene['front_axis']='-Y'
    scene['up_axis']='+Z'
    scene['item_id']=item
    ...
    for size in (16,32,64,128,1024):
        ...
        if size==128:
            bpy.data.images['Render Result'].save_render(str(ASSETS/'textures/item'/(item+'.png')))
    triangles=0
...
def sheets():
    ...
    q.sheet_text('APPROVED V2 / NATIVE RECONSTRUCTION',26,18,27)
...
def verify():
    ...
    for item in ITEMS:
        ...
        resource=ASSETS/'textures/item'/(item+'.png')
        ...
        if item=='micro_missile_pack':
            ...
            for x,z in ((-.53,1.37),(0,1.37),(.53,1.37),(-.29,.55),(.29,.55)):
                ...
                tip=bool(hit and loc.y<-.70 and obj.name.startswith('Missile_'))
                tips.append({'xz':[x,z],'hit':tip,'object':obj.name if obj else None})
            report['fivePhysicalNoses']=tips
...
if __name__=='__main__':
    ...
    parser.add_argument('--only',choices=(*ITEMS,'sheets','verify','all'),default='all')
    args=parser.parse_args(sys.argv[sys.argv.index('--')+1:] if '--' in sys.argv else [])
    for item in ITEMS:
        if args.only in ('all',item):
            build(item)
