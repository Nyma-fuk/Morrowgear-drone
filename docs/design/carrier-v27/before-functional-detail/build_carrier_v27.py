...
def flight_deck():
    ...
    for x,y in ((6.2,-11),(7.0,4.2)):
        box('DeckMaintenanceSeal',(x,y,4.72),(1.4,1.65,.08),'dark',True,.10,False)
        box('DeckMaintenanceLid',(x,y,4.775),(1.18,1.42,.055),'panel',True,.08,False)
...
def citadel():
    ...
    for y,z,w in ((-12,6.98,1.45),(-7.5,7.6,2.75),(-1,8.36,3.1),(13.4,8.8,1.2)):
        box('CitadelArmoredAccess',(0,y,z),(w,1.7,.08),'armor',False,.18,False)
        for x in (w*.30,):
            box('CitadelAccessLock',(x,y-.55,z+.065),(.10,.27,.05),'amber',True,.02,False)
    for x in (2.55,):
        for y in (-7,-2,3):
...
def finish_surfaces():
    ...
    for y in (-15,-10,14,18,21):
        width=3.1 if y<0 else 2.4
        floor=1.40 if y<0 else 2.12+(y-14)*.085
        box('BellyAccessRecess',(0,y,floor-.045),(width,2.2,.07),'dark',False,.16,False)
        box('BellyAccessArmor',(0,y,floor-.085),(width-.2,1.98,.05),'panel',False,.14,False)
        for x in (width*.3,):
            box('BellyCaptiveLock',(x,y-.7,floor-.12),(.11,.23,.04),'edge',True,.02,False)
    for y in (-11.5,-5.8,.2,6.2):
        box('BellyLongeron',(10.05,y,1.60),(.29,3.6,.18),'panel',True,.05)
        box('BellyCoolingRecess',(9.6,y,1.62),(1.25,2.05,.08),'dark',True,.12,False)
        for j in range(5):
            box('BellyCoolingLouver',(9.6,y-.70+j*.35,1.57),(1.02,.12,.10),'edge',True,.02,False)


def weapons_and_panels():
    for x,y,z in ((5.7,-17.2,5.52),(3.25,9.0,8.02)):
        cylinder('ArmoredDefenseMount',(x,y,z),.85,.30,'edge',True,16)
        box('DefenseTurret',(x,y-.12,z+.42),(1.2,1.65,.70),'armor',True,.22)
        for dx in (-.29,.29):
            cylinder('DefenseBarrel',(x+dx,y-1.20,z+.47),.105,1.50,'dark',True,12,True,'Y')
        box('DefenseOptic',(x,y-.96,z+.40),(.20,.04,.14),'glass',True,.01)
    for y in (-11.0,-5.4,.2,5.7):
        outer=min(12.5,12.15+(y+11)*.016)
        outline=[(9.83,y-1.9),(outer-.35,y-1.25),(outer,y+.75),(10.0,y+1.5)]
        panel('OuterPanelJoint',outline,'dark',.025)
        cx=sum(p[0] for p in outline)/4
        cy=sum(p[1] for p in outline)/4
        inset=[(cx+(x-cx)*.91,cy+(yy-cy)*.92) for x,yy in outline]
        panel('OuterArmorPanel',inset,'panel',.063)
        for x,yy in (inset[0],inset[2]):
            cylinder('PanelCaptiveLock',(x,yy,skin(x,yy)+.10),.075,.035,'edge',True,8,False)
...
def build():
    ...
    v.M['panel']=v.mat('CarrierCastPanel',(.10,.12,.14),.6,.42)
    ...
    ring('EmitterArmoredRing',(0,0,.44),2.7,2.21,.52)
    cylinder('IntegratedEmitterLens',(0,0,.13),2.20,.20,'glass',False,48)
    ring('EmitterBellyDatum',(0,0,.08),1.35,1.23,.16,'dark')
    for i in range(12):
        a=i*math.tau/12
        x,y=2.48*math.cos(a),2.48*math.sin(a)
        box('EmitterCyanSegment',(x,y,.15),(.14,.12,.03),'cyan',False,.02)
    q.canonical_halves()
    ...
    render_views()
    contract={'revision':'V27_APPROVED_DESIGN_50','runtimeBounds':[-13,0,-25,13,11,25],
...
def validate():
    ...
    return {'passed':not errors,'errors':errors,'boundsBlender':bb,'mirrorError':error,'triangles':tris,
            ...
            'scope':'Native geometry, 676 entry rays, no runtime game session or moving mechanism test'}
...
def render_views():
    ...
    for size in (16,32,64,128,1024):
        ...
        if size==128:
            destination=ASSETS/'textures/item/carrier_unit.png'
...
def export_runtime(lod=False):
    ...
    atlas_path=ASSETS/'textures/runtime'/(name+'.png')
    atlas_path.parent.mkdir(parents=True,exist_ok=True)
    atlas.filepath_raw=str(atlas_path)
    atlas.file_format='PNG'
    atlas.save()
    mesh_path=ASSETS/'models/runtime'/(name+'.mgm')
    mesh_path.parent.mkdir(parents=True,exist_ok=True)
    mesh.calc_loop_triangles()
    count=len(mesh.loop_triangles)*3
    if count>75000:
        raise ValueError('Carrier 25k triangle budget exceeded')
...
def read_runtime(path):
    ...
    return {'passed':not errors,'errors':errors,'format':'MGM4','vertices':count,'triangles':count//3,
            ...
            'rotorCenters':2,'groups':[0],'posesIdentical':True,'luminousVertices':luminous}
...
def verify_outputs():
    ...
    for name in ('carrier','carrier_lod'):
        path=ASSETS/'models/runtime'/(name+'.mgm')
        ...
        bpy.context.scene.collection.objects.link(obj)
        atlas=bpy.data.images.load(str(ASSETS/'textures/runtime'/(name+'.png')))
        for signal in (0,1,2):
    ...
    q.sheet_text('MGM4: 18,688 tris / LOD: 9,712 tris',24,1250,22)
