...
final class AdaptiveFollowFormation {
	...
	static Profile profile(int count, double openness, int ceilingClearance,
		...
		double radius = column
			? 1.55 + open * 1.2
			: 2.8 + Math.sqrt(Math.max(1, count)) * 0.72 + speed * 1.5;
		double maximumRadius = Math.max(2.6, 7.4 - Math.max(0, count - 8) * 0.06);
		...
		double baseHeight = column
			...
			: 2.35 + open * 0.8 + speed * 0.75 + (fluidNearby ? 1.0 : 0.0);
		...
		double angularSpeed = column ? 0.0 : Mth.clamp(0.042 - count * 0.0014 - speed * 0.006, 0.018, 0.038);
	...
	private static Vec3 offset(Profile profile, int index, int count, long tick,
		...
		if (profile.column()) {
			int row = index / 2;
			double side = index % 2 == 0 ? -0.85 : 0.85;
			return forward.scale(-1.5 - row * 1.8)
				.add(right.scale(side))
				.add(0, profile.baseHeight() + (index % 3) * profile.verticalSpacing(), 0);
		}
		...
		double largeFleetLayer = count >= SwarmFormation.HIERARCHICAL_FLEET_SIZE ? layer * 3.2 : layer * 0.35;
